...
export const RouteStopsService = {
    ...
    async getAllRouteStopsByRouteId(route_id: number): Promise<RouteStop[]> {
        const query = "SELECT * FROM route_stops WHERE route_id = $1 ORDER BY stop_order ASC";
        const values = [route_id];
        const result = await db.query(query, values);
        return result.rows;
    },

    async addRouteStop(route_id: number, location_id: number, stop_order: number, notes: string, meetup_time: Date | null): Promise<RouteStop> {
        const query = `
    ...
    async updateRouteStop(route_id: number, original_stop_order: number, location_id: number, new_stop_order: number, notes: string, meetup_time: Date | null): Promise<RouteStop> {
        const query = `
            UPDATE route_stops
